...
A switch can be used to indicate and toggle the state of a binary state. Popular use-cases inlcude switching dark mode on or off, switching WiFi on or off and extends to any kind of setting where a user is limited to a binary state - true or false, on or off.
...
- What should the active or checked state of the switch be called?
  This could be used in multiple places, also a CSS pseudo class `switch:toggled`.
...
| `checked-track` (?)   | Enables inserting content on the checked (end) side of the track     | No default content                          |
| `unchecked-track` (?) | Enables inserting content on the unchecked (start) side fo the track | No default content                          |
...
- Should the switch be form associated?
  Popular usage guidelines recommend a button-like usage with immediate effects after interaction instead of after form submission. Usage in a form is generally discouraged.
...
- The current value should be reported as "on" or "off" to scree readers
