...
- [The WebKit standards position on interestfor](https://github.com/WebKit/standards-positions/issues/464)
...
It is handy to be able to select elements that both a) have the `interestfor` attribute, and b) are *currently* being shown interest. The `:has-interest` pseudo class matches elements in exactly this state. Two common use cases arise:
...
container:has(:has-interest) [interestfor] {
