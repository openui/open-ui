...
- [HIDs and Interest](#hids-and-interest)
  ...
  - [Touchscreen](#touchscreen)
    ...
    - [Option 5 - invent a new gesture](#option-5---invent-a-new-gesture)
    - [Option 6 - do not show the context menu](#option-6---do-not-show-the-context-menu)
  - [Other](#other)
...
![Github profile hovercard](/images/github-profile-hovercard.png)
...
While an invoker is activated in "partial interest" mode, it is still possible for a user that also uses a mouse to try to interact with things in the target popover. For this reason, hovering a target popover with a mouse also causes the target to be "fully activated". For example, a user might tab to focus an `interesttarget` element, which shows a popover. Then they move their mouse over to a button within that popover, activating the popover and making the button clickable.
...
Unfortunately, this idea was [rejected by WebKit](https://github.com/WebKit/standards-positions/issues/305#issuecomment-2284319289), who wanted the exact behavior to be specified for touchscreen. This approach was apparently supported by [Mozilla](https://github.com/whatwg/html/issues/11058#issuecomment-2697565012).
...
This approach has been rejected by both [WebKit](https://github.com/whatwg/html/issues/11058#issuecomment-2676086663) and [Mozilla](https://github.com/whatwg/html/issues/11058#issuecomment-2695676452), as being too confusing to users and potentially a security issue (showing site and UA content together).

![Mockup of the "show both" approach](/images/interesttarget-mockup-show-both.gif)


### Option 3 - add an item to the context menu

This is likely the most straightforward approach that still meets all of the requirements: for elements that already have long-press-activated context menus, simply add an item to that menu (e.g. "More info about link from site"). When this item is tapped, dismisses the context menu and "show interest" in the link element. This approach does not affect the existing UX at all, and is straightforward to implement. It does represent a bit more friction for users, relative to desktop users: touchscreen users must first long-press the link, then single-tap a menu item, in order to show interest. However, this is still infinitely better than the current state of the art, which is to simply deny touchscreen users any access to the hovercard.

![Mockup of the "add context menu item" approach](/images/interesttarget-mockup-add-to-context-menu.gif)

### Option 4 - single tap interest

Another approach is to **not** use the long-press gesture at all, but instead make a single-tap on a link with `interesttarget` "show interest". A second tap on that same link (perhaps within a time delay?) would then navigate to the link. This side-steps the context menu problem entirely, since the user can still long-press the same link and get access to the context menu. However, it does introduce a bit of "tap uncertainty" for users. Simply looking at a link on the screen does not indicate to them whether a single-tap will navigate to a new page, or show interesting information. Without pre-exploring a site, it will be unclear whether the user is "safe" to tap links without leaving the site.

![Mockup of the "single-tap" approach](/images/interesttarget-mockup-single-tap.gif)

### Option 5 - invent a new gesture

Another approach would be to "invent" a new gesture that users can use to show interest. For example, long-press for a specific amount of time, or long-press and "wiggle". Or something else. While UX designers could get creative and come up with something intuitive, it's important to point out again that on the rest of mobile platforms, **long-press** is the expected/trained UX pattern for showing interest in something. Creating a brand new gesture that only works on a single set of apps (browsers) and is different from the entire universe of other mobile apps, creates an extreme discovery problem. Users will not be aware of this magic gesture, and it will go unused. It does however get around the conflict between showing interest and showing the context menu.

### Option 6 - do not show the context menu

One final approach is to simply *not show* the context menu on long press, for elements with `interesttarget`. Long-pressing the element triggers interest and opens the hovercard. There is no way for the user to access the context menu, but perhaps that's "ok", since the site has provided a highly-customized hovercard for the link that obviates the user's desire to navigate to the link. Still, it is likely that some users will be annoyed at the lack of functionality, e.g. their inability to copy the link URL to the clipboard.
