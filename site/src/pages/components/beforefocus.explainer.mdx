...
[focusgroup](./focusgroup.explainer)) will be useful, there are a multitude of different
applications for focus, and lower level primitives will give web developers more flexibility.

Instead of high level APIs, this proposal focuses on having two fairly simple lower level
functions which would arguably allow the application developers to implement whatever patterns
they prefer without working around the browser limitations and impossibilities.
...
For arrow keypresses and/or iframe situations we would able to use `document.focusNext()` (of course, extending
