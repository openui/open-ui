...
For keyboard input specifically, user agents are expected to suppress focusgroup directional navigation when an arrow key is pressed together with a modifier key (Ctrl, Alt, Meta, or Shift). This preserves modifier+arrow combinations for their established platform-specific behaviors, such as word-by-word text navigation with Ctrl+Arrow or text selection with Shift+Arrow. Other input modalities typically lack an equivalent "modifier" concept; the user agent's mapping for those modalities determines when directional navigation is triggered.

This keyboard-mapping rule is captured normatively in the [WHATWG focusgroup pull request](https://github.com/whatwg/html/pull/11723) — see the [modifier-key handling additions](https://github.com/whatwg/html/pull/11723/changes/1017afbf2920147b4658a986d71813936e431a57) for the specification text.
...
When focus is on a focusgroup item, focusgroup handles arrow-key movement within the segment; when the user moves focus out of the focusgroup (via Tab, or when the segment provides no further movement in the requested direction), spatial navigation can take over for document-level traversal. Initial author exploration on the focusgroup demo page suggests that the two features compose well in practice — focusgroup facilitating navigation within author-defined controls, and spatial navigation facilitating document-level traversal.

This composition is particularly relevant for devices that rely on directional input (TVs and remote controls, D-pads, game controllers) and for assistive technologies that expose arrow-key traversal as a primary input modality. Focusgroup intentionally does not attempt to specify spatial-navigation semantics beyond a focusgroup's own scope; the surrounding document's traversal model — whether sequential, spatial, or AT-driven — remains the responsibility of the user agent and any applicable specifications such as CSS spatial navigation.

Authors should take care that the [`wrap`](#enabling-wrapping-behaviors) behavior does not impede spatial navigation out of a focusgroup. With `wrap`, arrow-key navigation cycles within the segment indefinitely and never reaches the segment's edge, which can prevent a spatial-navigation algorithm from receiving a "no further item in this direction" signal that would otherwise let it move focus to a neighboring control outside the focusgroup. This is the same authoring consideration that applies today when implementing roving-tabindex composites in JavaScript: wrapping is appropriate for self-contained patterns whose users do not expect arrow keys to leave the widget (for example tabs and radiogroups), and is generally inappropriate for patterns that should hand off to surrounding navigation. Use [`nowrap`](#enabling-wrapping-behaviors) (or rely on a behavior token whose [default modifiers](#default-modifiers) do not include `wrap`) when the focusgroup needs to participate in document-level spatial navigation.
...
- [For keyboard input, user agents suppress focusgroup directional navigation when modifier keys are held, preserving platform behaviors such as Ctrl+Arrow and Shift+Arrow](https://github.com/whatwg/html/pull/11723/changes/1017afbf2920147b4658a986d71813936e431a57)
- [Focusgroup and CSS spatial navigation are complementary and expected to compose](https://github.com/whatwg/html/issues/11641#issuecomment-4028039082)
