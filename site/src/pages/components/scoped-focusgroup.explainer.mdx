...
* The semantic contract (composite widget → managed internal focus) is already implied.

Reasons for scoping now:
* Reasonable constraint: Prevents legitimizing arrow navigation on arbitrary layout containers that lack meaningful relationships, guiding developers to use patterns that are well defined (paving the cow paths).
* Author guardrails: Most real implementations (libraries/utilities) already concentrate on these composites; constraining the native feature mirrors current successful practice and reduces accidental misuse.
* Future evolution can consider additional roles or a role-agnostic extension when there is objective evidence (author demand + accessible patterns + testable semantics) that expansion improves user outcomes without reintroducing the risks above. Going the other way is nearly impossible.
...
### Eligible Roles
First token (container composite role) values:
* `toolbar`
* `tablist`
* `radiogroup`
* `listbox`
...
`focusgroup` aims to reduce boilerplate while nudging authors toward patterns described in the WAI-ARIA Authoring Practices (APG). When the first token is a recognized composite role and a managed descendant lacks an explicit `role`, the user agent MAY assign the corresponding expected item role. Overriding is ALWAYS allowed: an explicit child `role` attribute wins. The table also calls out common, legitimate reasons you might intentionally override.
