...
The grid, item-controls, and feed additions previously tracked here as future considerations are now proposed separately in the [Focusgroup V2 explainer](/components/focusgroup-v2.explainer).
...
| [`wrap`](#enabling-linear-wrapping-behavior) | Optional | Loops end→start and start→end. Some behavior tokens enable `wrap` by [default](#default-modifiers). |
| [`nowrap`](#enabling-linear-wrapping-behavior) | Optional | Explicitly hard edges. Overrides a behavior token's default `wrap`. Cannot be combined with `wrap`. |
...
|---|---|---|
| [`focusgroupstart`](#the-focusgroupstart-attribute) | An item in any focusgroup | Marks the item that receives focus on first sequential entry. Reflected by the `focusGroupStart` IDL property. |

## Differences from the original explainer

This proposal narrows the scope of the [original focusgroup explainer](https://github.com/openui/open-ui/blob/fdd348a/site/src/pages/components/focusgroup.explainer.mdx). This section summarizes what changed and the behaviors `focusgroup` supports.
...
The [ARIA in HTML specification](https://www.w3.org/TR/html-aria/#docconformance) defines which ARIA roles authors may use on each HTML element. Its [`button` entry](https://www.w3.org/TR/html-aria/#el-button) lists the roles a `<button>` may take, including `tab`, `radio`, and the `menuitem` variants. The focusgroup button exception follows that per-element list: it never infers a role that the specification does not allow on `<button>`.
...
- Tokens that don't appear in the default modifiers column behave as documented in [Enabling linear wrapping behavior](#enabling-linear-wrapping-behavior) and [Limiting linear focusgroup directionality](#limiting-linear-focusgroup-directionality) (i.e., no wrapping, both axes enabled).
...
5. Child role inference only runs when: (a) container role came from behavior mapping, (b) descendant is managed (not opted-out), (c) behavior defines an inferred child role, (d) descendant lacks explicit `role`, and (e) inference would not replace non-generic native semantics. The exception is `<button>`, which is eligible when the inferred role appears in the [ARIA in HTML `button` entry](https://www.w3.org/TR/html-aria/#el-button). Other native interactive elements retain their native semantics.
...
- Focus will [`wrap`](#enabling-linear-wrapping-behavior) from one end of the tablist to the other because `tablist` carries `wrap` as a [default modifier](#default-modifiers).
...
```
What to notice:
- In `#one`, the generic `<div>` has no explicit `role`, so the `menu` token supplies the container `menu` role via the [minimum role mapping](#supported-behaviors). The plain buttons (Bold and Underline) receive the inferred `menuitem` role; the explicit `menuitemcheckbox` (Italics) is unchanged. (Had `#one` been a non-generic element such as `<nav>` or `<ul>`, the container mapping would be skipped, and with it child inference; see [choosing your markup](#supported-behaviors).)
...
A `focusgroup` provides arrow key navigation among a _list_ of related elements.
...
("tab navigation"). Focusgroups ensure that **exactly one focusgroup item** serves as the
...
Authors should take care that the [`wrap`](#enabling-linear-wrapping-behavior) behavior does not impede spatial navigation out of a focusgroup. With `wrap`, arrow-key navigation cycles within the segment indefinitely and never reaches the segment's edge, which can prevent a spatial-navigation algorithm from receiving a "no further item in this direction" signal that would otherwise let it move focus to a neighboring control outside the focusgroup. Authors should consider using [`nowrap`](#enabling-linear-wrapping-behavior) or rely on a behavior token whose [default modifiers](#default-modifiers) do not include `wrap` when there is no target for spatial navigation in the cross-axis. The same authoring consideration applies today when implementing roving-tabindex composites in JavaScript.
...
Feature detection is generic token and property detection. Authors test whether the
`focusGroup` token list recognizes a behavior or modifier token, and whether a
...
`DOMTokenList.supports()` reports whether the user agent recognizes a token. It does
not validate a particular owner's roles or descendants. Unknown tokens are ignored
under the ordinary [token-list fallback](#index-of-focusgroup-values).

### Additional features

Focusgroups have the following additional features:

- [Wrap-around semantics](#enabling-linear-wrapping-behavior) - what to do when attempting to move past
   the end of a `focusgroup`. The default/initial value is `nowrap`, which means that focus is
   not moved past the ends of a `focusgroup` with the arrow keys.
- [Directional axis limits](#limiting-linear-focusgroup-directionality) - respond to arrow keys in one
   axis only (either up/down or left/right when the arrow key pressed matches the corresponding flow of
   the content). By default, `focusgroup`s respond to all four arrow keys.
...
|---|---|---|
| `focusgroup="<behavior>"` (where `<behavior>` is a linear behavior) | linear | No wrapping; edges are hard stops (unless the behavior token's [default modifiers](#default-modifiers) include `wrap`). |
| `focusgroup="<behavior> wrap"` (where `<behavior>` is a linear behavior) | linear | Moving past one end wraps focus to the opposite end. |
| `focusgroup="<behavior> nowrap"` (where `<behavior>` is a linear behavior) | linear | Explicitly disable wrapping; edges are hard stops. Overrides a behavior token's default `wrap` modifier. |
...
_Note: `<behavior>` below refers to a linear behavior._
...
Two-dimensional grid navigation for tables and other tabular structures is specified separately in the [Focusgroup V2 explainer](/components/focusgroup-v2.explainer). See [Grid focusgroups](/components/focusgroup-v2.explainer#grid-focusgroups) for the grid behavior, which builds on the linear focusgroup model defined here.
...
1. **Supported behaviors:** The [list of supported behaviors for `focusgroup`](#supported-behaviors) is open for discussion, and input on what should be allowed is welcome.
  - MDN documentation includes [a recommendation to **not**](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles) use `listbox` as a composite widget, but supporting it here would send a signal that it is OK to use it in this scenario.
  - Are there other roles not listed here that have a strong use case for `focusgroup`?
2. **Attribute functionality and dependencies:** Feedback requested on adopted scoping (role token + optional child role inference):
...
3. **Alternative approaches to scope focusgroup to specific scenarios:**
...
|---|---|
| Marks the item focused on first sequential entry; reflected as `focusGroupStart` | `focusgroupstart` |
