...
* Limits activation to a concise, enumerated set of established ARIA composite widget patterns (first token = composite role).
...
Benefits of this scoped, token-first approach:
...
Open aspects still under discussion appear in the Open Questions (e.g., set of supported patterns, child role inference).

### Supported Patterns
First token (container composite role) values (current pattern set):
* `toolbar`
* `tablist`
* `radiogroup`
* `listbox`
...
Child item role inference (automatic when author omits explicit child `role` attributes):

`focusgroup` aims to reduce boilerplate while nudging authors toward patterns described in the WAI-ARIA Authoring Practices (APG). When the first token is a recognized composite role and a managed descendant lacks an explicit `role`, the user agent MAY assign the corresponding expected item role. Overriding is ALWAYS allowed: an explicit `role` attribute will take precedence. The table also calls out common, legitimate reasons you might intentionally override.

| Composite token (first token) | Inferred child role(s) | Purpose / APG alignment | When you might override |
|---|---|---|---|
| `radiogroup` | `radio` | Matches APG radiogroup pattern (one roving tab stop, arrow movement among radios). | To mix a different toggle type (e.g. `role="switch"`) or introduce non-radio items. |
| `tablist` | `tab` | Encourages APG-compliant tabs controlling `tabpanel`s. | For hybrid controls where some items are disclosure buttons not acting as tabs. |
| `menu` / `menubar` | `menuitem` | Provides baseline APG menu / menubar semantics. | To specify `menuitemcheckbox` / `menuitemradio` or custom interactive roles. |
| `listbox` | `option` | Aligns with APG listbox option semantics. | For rich, grouped, or multi-role items (e.g. adding `role="option"` plus nested buttons). |
| `toolbar` | (none) | Leaves native semantics of existing interactive descendants. | Usually no need; add roles only for custom elements lacking native semantics. |
| `grid` (future) | (TBD) | 2-D navigation future; no inference defined yet. | Will depend on future design. |

#### Role inference behavior
User agents MAY infer roles to reduce boilerplate while preserving author control ([see open question #5](#open-questions)).
1. Explicit container `role` overrides any pattern token; no child inference occurs in that case.
2. Otherwise the first token (if a recognized pattern) supplies the container role.
3. [Child role inference](#open-questions) only when ALL are true:
  * Parent role came from the pattern token (no conflicting explicit role).
  * Descendant is managed by `focusgroup` (and not inside `focusgroup="none"`).
  * Pattern defines a child role for such descendants.
  * Descendant lacks an explicit `role`.
  * Assigning the inferred role would not replace richer native semantics already present (e.g., native `<button>` in a `toolbar`).
4. Ambiguity skip: if multiple roles would be equally valid (e.g., `menuitemcheckbox` vs `menuitemradio`), no inference is applied.
5. Explicit child roles always win (override silently); authors may add them for variants or to block inference.
6. Tooling MAY suggest removing redundant explicit roles when inference would apply.
7. Future: a modifier such as `no-infer` could disable child inference without forcing explicit roles (pending discussion).
...
2. Incremental flexibility: Adding a new eligible composite token (and optional child inference) is a far smaller spec and implementation change than introducing a new HTML element with parsing, styling, accessibility mapping, and legacy considerations.
...
Interactions with aria-role, also see [open question around child role inference](#open-questions)
```html
...
as a global attribute, its applicability is limited to a subset of pattens, requiring the author to
specify the pattern their control follows when using focusgroup, see: [supported patterns](#supported-patterns)
...
### Authoring guidance
1. Put the composite role token first (if used): `focusgroup="tablist wrap"`, `focusgroup="toolbar"`.
...
1. **Supported Patterns:** The [list of supported patterns for `focusgroup`](#supported-patterns) is open for discussion, and I am open to input on what should be allowed.
...
Tokens are space-separated. Order: first token MUST be a container composite role (eligible list below). Remaining tokens are modifiers. Unknown tokens are ignored.

***Container role tokens (first token):***

| Role | Implied container role if no explicit `role` attr | [Child role inference](#open-questions) (when role omitted) |
|---|---|---|
| `toolbar` | toolbar | none (native button semantics) |
| `tablist` | tablist | `tab` |
| `radiogroup` | radiogroup | `radio` |
| `menu` | menu | `menuitem` |
| `menubar` | menubar | `menuitem` |
| `listbox` | listbox | `option` |
| `grid` (future) | grid | none (2-D navigation future) |

Reminder: explicit container and child roles always override any inference described above.
