...
| `wrap` | Optional | Loops end→start and start→end. |
...
A **focusgroup segment** is a contiguous group of focusgroup items that can be navigated without crossing any [opted-out elements](#opting-out). When elements within a focusgroup use `focusgroup="none"` to opt-out, they effectively divide the focusgroup into separate segments for sequential focus navigation purposes.

Each segment operates independently for certain behaviors:
- **Memory:** Each segment maintains its own last-focused element memory
- **Guaranteed tab stop:** Each segment ensures at least one tab-accessible element
- **Sequential navigation:** Tab and Shift+Tab treat each segment as a separate focusgroup entry point

For example, if a focusgroup contains items A, B, C, D, E and item C opts-out, this creates two segments: [A, B] and [D, E]. Arrow key navigation can move freely within each segment and skip over the opted-out element C, but sequential focus navigation (Tab/Shift+Tab) will treat these as two separate tab stops.

See [Impact on sequential focus navigation](#impact-on-sequential-focus-navigation) for detailed behavior and examples.

### Last-focused memory

By default, [focusgroup segments](#focusgroup-segments) will remember the last-focused element, and for sequential focus navigation,
...
3. **Apply focus priority (first match wins):**
   - If the focusgroup has a **last focused item** and the [`no-memory`](#last-focused-memory) token is not present, and the last focused item is within the current segment, focus that item.
...
4. **Update tab navigation:** All other `focusgroup` items are removed from the sequential focus navigation order (e.g., by treating their `tabindex` as -1).

This algorithm ensures that each [focusgroup segment](#focusgroup-segments) has a [guaranteed tab stop](#guaranteed-tab-stop), maintaining accessibility even when all items have `tabindex="-1"`.
