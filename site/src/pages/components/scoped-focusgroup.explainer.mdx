...
Child item role inference (when role omitted):
* `radiogroup` ⇒ `radio`
* `tablist` ⇒ `tab`
* `menu` / `menubar` ⇒ `menuitem`
* `listbox` ⇒ `option`
* `toolbar` ⇒ (no special inference; native semantics retained)
...
* Exact list & phase-in of child role inferences.
* Console warnings vs silent ignore for mismatches.
* Opt-out token for inference (e.g. `no-infer`) if needed.
## Open Questions
...
4. **Attribute Functionality and Dependencies:** Feedback requested on adopted scoping (role token + optional child role inference):
  - Initial inference role set size (keep minimal vs broad?).
  ...
  - Telemetry/authoring data sources to validate expansion or adjustments.
