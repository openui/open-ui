...
- [Focusgroup Concepts](#focusgroup-concepts)
  ...
  - [Key conflict elements](#key-conflict-elements)
    - [Determining key conflict elements](#determining-key-conflict-elements)
...
<div style="height: 200px; overflow: auto;">
  <div focusgroup="toolbar inline wrap">
    <button>Bold</button>
    <button>Italic</button>
    <!-- More content that makes the next button off-screen -->
    <button>Underline</button>
  </div>
  <!-- More content that makes this scrollable -->
</div>
```

In this example:
- Left/right arrows navigate between toolbar buttons and wrap around
- Up/down arrows scroll the container since the focusgroup is limited to `inline`
- Focus is scrolled into view automatically when navigating between buttons

Note: Focusgroup is primarily intended for grouping related controls, so it is not recommended to use focusgroup on large scrollable lists or grids of items. Instead, use focusgroup on a toolbar or menu that controls the larger scrollable content. The above example is provided to illustrate the interaction between focusgroup and scrolling, and is **not** a recommended use of focusgroup.
