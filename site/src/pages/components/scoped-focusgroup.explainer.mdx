...
```
What to notice:
* In the first example, the author chooses to not specify the role, relying solely on the `focusgroup` attribute to convey the intended behavior. This would be the equivalent of using the `role` attribute, but with a more concise syntax.
...
### Conflicting role specifications
```html
