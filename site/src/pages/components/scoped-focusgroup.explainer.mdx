...
Notes:
* Overriding is always safe: the platform never blocks or second-guesses an explicit role.
* Inference is intentionally conservative; it never assigns a role where multiple equally valid choices exist (e.g., it won’t guess between `menuitemradio` vs `menuitemcheckbox`).
* If an inferred role would conflict with native semantics that provide richer accessibility (e.g., a native `<button>` inside a `toolbar`), no change is made.
* Tooling and validators can surface when explicit roles are unnecessary, helping authors simplify markup.
...
### Authoring guidance
1. Put the composite role token first: `focusgroup="tablist wrap"`, `focusgroup="toolbar"`.
2. Omit common child roles (radios in radiogroup, tabs in tablist, menuitems in menubar/menu, options in listbox, buttons in toolbar) unless you need an override.
3. Avoid mismatches: if you also supply a `role` attribute on the container and it conflicts with the first token, the explicit `role` wins; UAs MAY warn.
4. Explicit child roles override inference silently—use when combining heterogeneous items.
5. Inference never assigns roles where multiple valid item roles could apply ambiguously.
...
2. Parent-only implicit role (earlier alternative): saves one attribute but leaves child role repetition; weaker accessibility guardrails.
3. Adopted (parent token + child inference): best balance of ergonomics, clarity, guidance.
## Open Questions
...
4. **Attribute Functionality and Dependencies:** Feedback requested on adopted scoping (role token + optional child role inference):
  ...
  - Need an explicit opt-out token (e.g., `no-infer`) for child inference?
...
| Role | Implied container role if no explicit `role` attr | Child role inference (when role omitted) |
|---|---|---|
