...
name: Focusgroup (Scoped Explainer)
...
- [Proposal](#proposal)
  - [Configuration Tokens](#1-configuration-tokens)
  - [Scope & Eligible Items](#2-scope--eligible-items)
  - [Entry](#3-entry-single-tab-stop)
  - [Memory](#4-memory)
  - [Navigation](#5-navigation-linear-widgets)
  - [Grid Navigation](#6-grid-navigation-grid-token)
  - [Shadow DOM & Nesting](#7-shadow-dom--nesting)
  - [Conflict Handling](#8-conflict-handling)
  - [Algorithm Sketch](#9-focus-movement-algorithm-informative-sketch)
  - [Feature Detection](#10-feature-detection--progressive-enhancement)
  - [Migration Example](#11-migration-example)
  - [Failure Modes](#14-failure-modes-explicit)
- [Alternatives Considered](#alternatives-considered)
- [Accessibility](#accessibility-screen-reader--keyboard-semantics)
- [Security & Privacy](#security--privacy-considerations)
- [Implementation Notes](#implementation-guidelines)
- [Testing](#testing--quality-assurance)
- [Future Expansion](#future-expansion)
- [Open Questions](#open-questions)
- [Appendix](#appendix-syntax-quick-reference)
- [Issues / Discussions](#issues--discussions)
- [Glossary](#glossary)
- [Conclusion](#conclusion)
## Introduction

Authors routinely hand-code “roving tabindex” logic for composite widgets (toolbars, tablists, menus, listboxes, grids). In practice this means providing a single tab stop to enter the control and then using arrow keys to move focus across the items. A broad earlier `focusgroup` explainer generalized this to all elements, but unconstrained usage would be easy to misapply to arbitrary containers, causing accessibility problems. The scoped version here intentionally limits the attribute to a small, well-defined set of ARIA composite widget roles  and their expected interactive descendants—preserving ergonomics while reducing accidental harm.
...
* menu
* menubar
* [(consideration for future) grid](#future-consideration-grid-focusgroups)
...
1. (Cross Shadow DOM) Focusable elements contained inside a Shadow DOM are discoverable and focusable when their Shadow Host or an ancestor element declares a focusgroup.
...
1. (Limit directional arrow keys) A focusgroup can be configured to respond to either the logical
   ...
   directions.
1. (Focus movement arrow keys follow content direction) The user's arrow key presses move the focus forward or backward in the DOM according to the writing mode and directionality of the content.  E.g., in RTL, an Arrow-Left key moves the focus forward according to the content direction.
1. (Opt-out) Individual elements can opt-out of focusgroup participation.
1. (Grid) Focusgroups can be used for grid-type navigation (`<table>`-structured content or other grid-like structured content).

A use case we are evaluating:
* (Grid) Focusgroups can be used on elements with `display: grid` to provide 2d grid navigation.
...
as a global attribute, it's applicability is limited to a subset of elements. Only elements with the
...
* [Issue 995](https://github.com/openui/open-ui/issues/995)
...
## Original Explainer
The original [focusgroup explainer](/components/focusgroup.explainer) allows
for the attribute to be applied to any element, and also details CSS attributes
that can be used to apply focusgroup.
