...
- The popover API comes with some **"nice to haves"**:
...
As written above, the `<div>` will be rendered `display:none` by the UA stylesheet, meaning it will not be shown when the page is loaded. To show the popover, one of several methods can be used: [declarative triggering](#declarative-triggers), [JavaScript triggering](#javascript-trigger).
...
When a popover is open, it will match the `:popover-open` pseudo class:

```javascript
const popover = document.createElement('div')
popover.popover = 'auto'
popover.matches(':popover-open') === false
popover.showPopover()
...
> 2. `N` is a descendant of another popover `P2`, which has an `anchor` attribute whose value is equal to `P`'s `id` or any of `P`'s flat-tree descendant's `id`s.
...
- [Open UI and the Popover API – utilitybend.com](https://utilitybend.com/blog/open-ui-and-the-popover-api)
