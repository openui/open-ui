...
When doing so, the default button, `<selectedvalue>`, and `<listbox>` are created for you. As with the current `<select>` element, the initial `<option>` will be returned as the `<selectedvalue>`.
