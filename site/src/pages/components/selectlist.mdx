...
- button (slot) - The portion of the element which is always visible and in flow with the page, unlike the listbox. It should contain a button to open the listbox. If this part is not provided by the author, then selectlist will automatically create one. All child elements of the selectlist, except `<listbox>`, `<option>`s, and `<optgroup>`s will be slotted into this slot.
...
<!-- TODO: Should <button type=selectmenu> come with a ::marker thing?
     If so, should selectmenu::marker and button::marker style it? -->
