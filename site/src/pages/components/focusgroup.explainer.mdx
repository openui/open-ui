...
1. focus navigation ([not selection](#non-goals)) using keyboard directional arrow keys (for lists or grids)
...
web platform's native [radio button group](https://html.spec.whatwg.org/multipage/input.html#radio-button-group)
with a custom control using the [radio group pattern](https://www.w3.org/WAI/ARIA/apg/patterns/radio/).
...
In this next example, the author is creating a 
...
<ul role=menubar aria-label="Mythical University" focusgroup="inline wrap">
  <li role=none>
    ...
    <ul role=menu focusgroup="block wrap" id=aboutpop aria-label=About tabindex=-1 popover>
  ...
  <li role=none>
    ...
    <ul role=menu focusgroup="block wrap" id=admitpop aria-label=Admissions tabindex=-1 popover>
      ...
      <li role=none>
        ...
        <ul role=menu focusgroup="block wrap" id=tuitpop aria-label=Tuition tabindex=-1 popover>
...
3. (Cross Shadow DOM) Focusable elements nested inside a Shadow DOM are discoverable and focusable
   when their Shadow Host or an ancestor element declares a focusgroup.
...
use a focusgroup, focus must enter that element's focusable shadow-inclusive children through 
pre-existing means, e.g., for accessibility and keyboard-only scenarios, the Tab key is typically 
used--programmatic calls to `element.focus()` or user clicks with a pointing device are alternatives.
...
developers to support focusgroup behavior across component boundaries. (Component authors that want to 
[opt-out of this behavior](#opting-in--opting-out) can do so.)
...
### Expanding and connecting linear focusgroups together (`extend`)

A focusgroup definition's focusgroup candidates are its direct shadow-inclusive children. There are two
...
### Interactions with directionality + extending
...
#### Axis-aligned extending linear focusgroups
...
#### Orthogonal-axis extending linear focusgroups
...
#### Notes about wrapping while extending
