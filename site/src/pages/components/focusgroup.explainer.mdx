...
* Placement of focus on the menus (the nested focusgroups) from the menubar is not a feature of 
   `focusgroup` (nested focusgroups are completely independent of their parent focusgroup). In this
...
JavaScript event handlers needed)! The arrow keys controlling the navigation are mapped 
...
items. Focusgroup items are the elements that actually participate in the focusgroup (among the 
possible focusgroup candidates). The element with a focusgroup definition is also a focusgroup
candidate. Focusgroup candidates include all of that element's shadow-inclusive descendants. 
...
The `parent` element has the **focusgroup definition**. The elements with `id=one`, `two`, and 
`three` (and any other shadow-inclusive descendants of `parent` that may be added or removed) are 
...
Focusgroups have the following features:
...
- [Last-focused opt-out](#last-focused-memory) - prevent the focusgroup from remembering what the 
...
- This focusgroup has no "memory" of a last-focused element within (e.g., it has not been entered
   before).
- Since neither of the above cases chose an alternate element, then the first focusgroup item in
