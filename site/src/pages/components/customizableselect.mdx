...
This example uses the customizable `<select>` element with custom CSS to target it which renders the option differently in the listbox vs in the button. The contents of each `<option>` have a `<span aria-hidden=true>` in order to make the option get announced the same whether its copied into the `<selectedcontent>` or being rendered in the dropdown.
