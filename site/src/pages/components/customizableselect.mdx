...
To ensure the behavior described above, the nested `<button>` is rendered `inert`. It is also set to `display:contents` by the UA stylesheet, so that there aren't two sets of box decorations. And due to `inert`, the `<button>` is pruned from the a11y tree, including descendants. So ARIA or HTML attributes on the `<button>` have no effect on the a11y tree.
