...
The `selectedcontentelement` attribute is an IDref which points to a single `<selectedcontent>` element to update. This allows the `<selectedcontent>` to exist outside of `<select>` in order to support the split other use cases.
...
The select element supports the `:open` pseudo selector as specified by CSS here: https://drafts.csswg.org/selectors/#open-state
