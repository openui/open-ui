...
Each effective owner removes its [eligible nested content](#eligible-nested-content) from inactive items. Nested content associated through more than one independent nested-focusgroup lookup remains available only when every association is active. Unassociated opted-out content remains an ordinary V1 segment boundary, and top-layer content remains excluded. This preserves the standalone-content case discussed in [Open UI issue #1489](https://github.com/openui/open-ui/issues/1489).
...
A remembered item remains storage-valid only while it remains in the relevant shadow-inclusive tree, is owned by the same focusgroup, remains an item of the selected behavior, and remains [focusable](https://html.spec.whatwg.org/multipage/interaction.html#focusable), [being rendered](https://html.spec.whatwg.org/multipage/rendering.html#being-rendered), enabled, and non-inert. Focusability here includes programmatic focusability and does not require sequential focusability. Losing any condition clears the item. Restoration additionally requires a storage-valid item to belong to the segment being entered and satisfy that behavior's entry requirements.
