...
1. `itemcontrols`, which keeps controls inside the active item in sequential focus navigation and removes controls inside inactive items.
...
For that owner, **eligible nested content** is either a focusable area in a `focusgroup="none"` subtree or the sequential boundary of an independent nested focusgroup that:

1. Maps to one of the owner's items through the lookup below.
2. Would already participate in sequential focus navigation under HTML and V1.

Disabled, inert, non-rendered, and negative-`tabindex` content is already absent and therefore does not qualify. `itemcontrols` never makes content focusable.

To find the containing item, the user agent tests the opted-out focusable area or nested focusgroup owner, then walks CSS flat-tree ancestors toward each enclosing owner with effective `itemcontrols`. Relevant opt-out and nested-scope boundaries are transparent for this lookup, but top-layer boundaries and opt-out ancestors that exclude the starting content stop it. A linear owner returns its first owned item. A valid grid returns the cell target without leaving the owned cell; the target need not be an ancestor.

The **active focusgroup item** is the currently focused owned item, or the item found for the currently focused nested content. Activity follows focus and is owner-relative. Pointer or script focus on associated negative-`tabindex` content can therefore make an item active without adding that content to sequential focus navigation.

Each effective owner removes this content from its inactive items. Content remains only when every enclosing effective owner associates it with an active item. Unassociated opted-out content remains an ordinary V1 segment boundary, and top-layer content remains excluded. This preserves the standalone-content case discussed in [Open UI issue #1489](https://github.com/openui/open-ui/issues/1489).
...
When memory is enabled, each effective owner stores its owner-relative active item, never the focused nested control. Memory updates after focus changes and relevant state reevaluation; no active item leaves valid prior memory unchanged. A remembered item is cleared if it leaves the relevant tree or ceases to be an eligible item of the same owner; restoration still requires it to belong to the segment being entered. `nomemory` applies only to its owner.
...
* An independent nested focusgroup retains its own scope. Its sequential boundary uses the same active-item filter as opted-out controls, resolving the composition case discussed in [Open UI issue #1489](https://github.com/openui/open-ui/issues/1489).
