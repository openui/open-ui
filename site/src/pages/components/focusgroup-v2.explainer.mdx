...
<p class="question">Should a linear focusgroup support a mode in which peer items remain reachable by both sequential and directional navigation? See [Open UI issue #1435](https://github.com/openui/open-ui/issues/1435).</p>
...
<p class="question">How does outer directional navigation resume while focus is inside a nested control, especially when that control consumes the same directional input? See Open UI issues [#1396](https://github.com/openui/open-ui/issues/1396) and [#1404](https://github.com/openui/open-ui/issues/1404).</p>

<p class="question">**Resolved in part:** An independent nested focusgroup retains its own scope, while its sequential boundary uses the same active-item filter as opted-out controls. Directional escape and resuming outer movement remain open; see Open UI issues [#1396](https://github.com/openui/open-ui/issues/1396) and [#1404](https://github.com/openui/open-ui/issues/1404).</p>

<p class="question">**Resolved in part:** Native key-conflict cell targets use HTML sequential focus navigation for Tab escape; `grid` coordinates do not redefine that order. Does focus inside nested controls require additional grid-specific escape behavior? See Open UI issues [#1396](https://github.com/openui/open-ui/issues/1396) and [#1404](https://github.com/openui/open-ui/issues/1404).</p>

### grid

<p class="question">How should a future grid extension represent ragged rows, row and column spans, and indexed topology? Should a future mode represent gaps as implicit empty coordinates or skip them, and how should it preserve rectangularity, block-axis columns, boundary operations, and memory?</p>
