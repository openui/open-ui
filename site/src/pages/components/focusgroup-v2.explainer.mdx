...
Role inference follows the same per-element constraints as V1. Because [ARIA in HTML](https://www.w3.org/TR/html-aria/#el-button) does not yet permit `gridcell` on `<button>`, a native control uses a generic cell wrapper. The wrapper receives the inferred role while the control can represent that cell during directional navigation.
