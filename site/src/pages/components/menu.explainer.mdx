...
- [Question and Answers](#question-and-answers)
  - [When to use Select vs Menu](#when-to-use-select-vs-menu)
  - [Why are we adding `<menuitem>` and not re-using the `<command>` element?](#why-are-we-adding-menuitem-and-not-re-using-the-command-element)
  - [Why are we adding `<menubar>` and `<menulist>` and not re-using the `<menu>` element?](#why-are-we-adding-menubar-and-menulist-and-not-re-using-the-menu-element)
  - [Popover semantics for `<menulist>`](#popover-semantics-for-menulist)
  - [Why are we using popover instead of openable for `<menulist>`?](#why-are-we-using-popover-instead-of-openable-for-menulist)
  - [What should be the keyboard behavior?](#what-should-be-the-keyboard-behavior)
  - [Should clicking/activating a menuitem close the menu by default?](#should-clickingactivating-a-menuitem-close-the-menu-by-default)
  - [Should any of these elements be form-associated?](#should-any-of-these-elements-be-form-associated)
  - [How can users customize the UI of these menu elements?](#how-can-users-customize-the-ui-of-these-menu-elements)
  - [There was a proposal for a Context Menu years ago. How is this different?](#there-was-a-proposal-for-a-context-menu-years-ago-how-is-this-different)
  - [How can we use menu elements to support custom right click Context Menus?](#how-can-we-use-menu-elements-to-support-custom-right-click-context-menus)
  - [How can we use menu elements to support navigation menus?](#how-can-we-use-menu-elements-to-support-navigation-menus)
  - [How can we use menu elements to support toolbars?](#how-can-we-use-menu-elements-to-support-toolbars)
  - [How can we use menu elements to support Touch Bar?](#how-can-we-use-menu-elements-to-support-touch-bar)
- [Examples in code](#examples-in-code)
  - [Google Docs: nested menulist](#google-docs-nested-menulist)
...
* May have a popovertarget which takes the idref of a submenu to show
    * Example: `<menuitem popovertarget=submenu>`
...
1. Add a type attribute: `<menuitem>`, `<menuitem type=checkbox>`, `<menuitem type=radio>`.
    1. If type is checkbox or radio, it is automatically checkable
2. Add a **checkable** attribute on the `<menuitem>`
    2. menuitem.checked IDL property to check if the item is checked
3. Add a **checkable** attribute on the grouping element (fieldset, menulist)
    3. Checkable attribute can be multiple (for checkbox) or single (for radio)
    4. `<fieldset checkable=multiple><menuitem>`
    5. `<fieldset checkable=single><menuitem>`
4. Add **name** attribute to group items together if we only want one element to checked at a time (for type radio)
        1. `<menuitem checkable name=foo>`
        2. `<menuitem checkable name=foo>`
...
Yes. While, we can use the <code>["name"](https://html.spec.whatwg.org/C#attr-fe-name)</code> attribute to logically group exclusive `<menuitemradio>` items together, to support captioned groups of `<menulist>` items, we should use the existing `<fieldset>` element (and its optional labeling `<legend>` element) that give this functionality to forms.

Furthermore, `<fieldset>` is a more general grouping element than `<optgroup>`, which we briefly considered, but decided against since it is used for grouping options.
...
If multiple `<menuitem>`s are added without a fieldset, they are grouped together under the parent `<menulist>`.
...
Similarly to customizable select, we expect everything to be wrapped inside `<menuitem>` according to the content model. This includes non-interactive elements like logo, images, text.
...
### How can users customize the UI of these menu elements?
...
* `<menulist>` can support more than top to bottom layout:
    * Set CSS display: grid and use rows/columns pattern.
