...
A navigation menu is for navigation to different pages on a site. It is often created using a `<nav>` element, which maps to the ARIA role `navigation`. Navigation menus do not need ARIA to be accessible. Since all items in a navigation menu are focusable links, users can depend on the focusable state to navigate between the different items. Navigation menus are the most common type of menus we see online. It can be found on blogs, SPA, news sites, anywhere. Most are following a **Disclosure Navigation Menu** pattern although it is also possible to implement it using a **Navigation Menubar** pattern (for example, to support nested menu lists).

We are evaluating the possibility to include **Navigation Menubar** pattern in our proposal. Currently, screen readers don't expect `<menuitem>` to be navigatable. We need to carefully evaluate if this is a valuable addition and how menuitem can be changed to support a navigation menubar.
