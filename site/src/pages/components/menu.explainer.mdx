...
application menus on the web. These menus will:
 ...
 - Represent commands as `<menuitem>`s, grouped together in `<menulist>` popovers or in-page
   `<menubar>`s.
...
* Supports the [`toggle`
  event](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/toggle_event), as it is a
  native **popover**.
...
* Provides the implicit [`menuitem`](https://w3c.github.io/aria/#menu) ARIA role.
    * Provides the implicit [`menuitemcheckbox`](https://w3c.github.io/aria/#menuitemcheckbox)
      ARIA role when nested in a `<fieldset checkable=multiple>`
    * Provides the implicit [`menuitemradio`](https://w3c.github.io/aria/#menuitemradio) ARIA role
      when nested in a `<fieldset checkable=single>` will have the
...
* Add a `checkable` IDL attribute on `<menuitem>`
    ...
    * Use a `name` attribute to group exclusive menu items together (this only applies for "radio"
      menu items):
        ...
        * `<menuitem checkable name=foo>`
* Add a `checkable` IDL attribute on the *grouping* element (for now, proposed as `<fieldset>`)
...
Question: What about reusing the `<menu>` element?

There is an existing [`<menu>`
element](https://html.spec.whatwg.org/multipage/grouping-content.html#the-menu-element), which
represents a "toolbar" listing of commands. Though it is named "menu", it is not programmatically
exposed like a "menu" or "menubar". There is also no native support to make listed commands look and
behave cohesively. This proposal introduces new menu elements to give web developers more
flexibility while following the [ARIA practices on menu and
menubar](https://www.w3.org/WAI/ARIA/apg/patterns/menubar/).

Our proposal does not modify the existing [`<menu>`
...
Question: Why not introduce new `<menuitemcheckbox>` and `<menuitemradio>` elements?

"instead of introducing new `<menuitemcheckbox>` and `<menuitemradio>` elements. We envision the
following markup:"

These represent ARIA roles, and could be exposed as elements, but this makes the proposal more
complicated by adding more elements. We get the accessibility wins.

Question: Is <fieldset> necessary to group <menuitem> together?

*Fieldset alternatives*:
 - We briefly considered using the [`name`](https://html.spec.whatwg.org/C#attr-fe-name) attribute
   to group exclusively checkable menu items, similar to how radio form inputs work. However, we
   don't need the `name` attribute if we're also grouping elements under a common `<fieldset>`.
...
Our goal is to make the menu elements we propose accessible by default, by implementing the
accessibility best practices gathered from the [ARIA Authoring Practices Guide
(APG)](https://www.w3.org/WAI/ARIA/apg/), and curated from accessibility experts. This section
describes the concrete implications on our proposal.

### Native ARIA role mapping

 - The `<menubar>` element will expose the [`menubar`](https://w3c.github.io/aria/#menubar) ARIA
   role
 - The `<menulist>` element will expose the [`menu`](https://w3c.github.io/aria/#menu) ARIA role
 - The `<menuitem>` element will, by default, expose the
...
Using the menubar pattern as an in-page navigation component has longstanding accessibility issues.
While we think it could be possible to address them to the satisfaction of the accessibility
community, and users of accessibility technology, this usage of menus is still very much under
discussion. See [§ How we can use menu elements to support navigation
menus](#how-can-we-use-menu-elements-to-support-navigation-menus), and
