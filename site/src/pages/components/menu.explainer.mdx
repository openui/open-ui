...
as a keyboard interaction & focus model that users expect.
...
`<fieldset>`, reflected by a corresponding boolean **IDL** attribute, to group items as either
radios or checkboxes; its allowed values are `checkable=single` and `checkable=multiple`:
...
### Native ARIA mapping rules

The elements and attributes are intended to provide native semantics that map to existing ARIA roles and states according to the following rules:

1. `<menubar>` elements have the [`menubar`](https://w3c.github.io/aria/#menubar) role.
2. `<menulist>` elements have the [`menu`](https://w3c.github.io/aria/#menu) role.
3. `<menuitem>` elements *usually* have the [`menuitem`](https://w3c.github.io/aria/#menuitem) role.
4. `<menuitem>`s can have [`menuitemradio`](https://w3c.github.io/aria/#menuitemradio) or [`menuitemcheckbox`](https://w3c.github.io/aria/#menuitemcheckbox) roles depending on the `checkable` attribute of a parent `<fieldset>`.
   - While the `<menuitem>` element's native ARIA role defaults to `menuitem`, it is responsive to whether the element is embedded in a `<fieldset checkable=single|multiple>`. The [`menuitemradio`](https://w3c.github.io/aria/#menuitemradio) role is exposed for items in a `<fieldset checkable=single>`, and the [`menuitemcheckbox`](https://w3c.github.io/aria/#menuitemcheckbox) role is exposed for items in a `<fieldset checkable=multiple>`.
   - `menuitemradio` and `menuitemcheckbox` elements expose a boolean `checked` state.
5. `menuitem`s and `button`s that are `menu` invokers:
   - have `expanded=true` or `expanded=false` state.
   - have `haspopup=menu`.
   - will NOT have `details` pointing to the `menu` ([past discussion on `haspopup` instead of `details`](https://github.com/openui/open-ui/issues/1297#issuecomment-3376865931)).
6. `menubar` and `menu` have `setSize`.
7. `menuitem` descendants of a `menubar` or `menu` have `setSize` and `posInSet`.
8. `menuitem`, `menuitemcheckbox`, and `menuitemradio` are included in the accessibility tree even when they are disabled.
...
In short, there are longstanding accessibility issues with navigation menus and our proposal does
...
* **Precedents**:
    * `<ul>`/`<ol>`
    * `<select>` and `<option>`
    * [Material Web menu component](https://github.com/material-components/material-web/blob/main/docs/components/menu.md) (avoids nested interactives)
    * [React Spectrum v3 Menu component](https://react-spectrum.adobe.com/v3/Menu.html) (avoids nested interactives)
    * [Semantic UI Menu collection](https://semantic-ui.com/collections/menu.html) (has nested interactives)
* **Advantages** (relative to both other options):
...
* **Disadvantages** (relative to both other options):
    ...
    * If the tree traversed is the light DOM, then harder or impossible to use across shadow DOM boundaries.  (Either alternative can be used with [`referenceTarget`](https://github.com/WICG/webcomponents/blob/gh-pages/proposals/reference-target-explainer.md).
...
GitHub: [Issue #1189](https://github.com/openui/open-ui/issues/1189)

We want to be able to represent two types of checkable menuitems:

 * exclusive checkable menuitems, where only one item in a group can be checked, typically represented with radio buttons
 * multiply-checkable menuitems, where multiple items in a group can be checked (although there might be just one item), typically represented with checkboxes

There are two high-level approaches for doing this:

 * with an attribute on each menu item, plus a name to group the exclusive items
 * with a grouping element that contains the items

The advantage of a grouping element is that it pushes content authors
to follow good practice for exclusive checkable menuitems,
which is that all of the items in a group should be together
with no other items between them (though separators are allowed).
It also pushes authors to avoid mixing checkbox and radio items together.
(Controlling whether an item is a radio or a checkbox with attributes
on the item, rather than with attributes on the grouping element,
makes it easier for authors to do mix items in ways that are bad practice.)

We would also like that grouping element to be a `<fieldset>`
because in some cases it makes sense to label a group of menuitems,
and `<fieldset>` provides an existing mechanism to do that.
In the typical case we would expect such a label to be visible and also be
exposed to assistive technology when navigating through the menuitems.
In some cases it may also be desirable for
the label to be exposed only to assistive technology but not be visible.
(This labeling may also apply to menuitems that are not checkable;
thus `<fieldset>` should be usable around other menuitems as well.)

Thus the current proposal introduces a single `<menuitem>` element
to represent all menu items,
including checkable state-bearing ones.

Instead of representing both checkable & non-checkable menu
...
These alternatives all have the disadvantages stated above that they make it easier
for authors to do things that are bad practice.
They don't appear to have any significant advantages,
except for the case of an isolated checkbox menuitem
which in our proposal requires the additional `<fieldset>` container.
These proposals would also require the additional complexity of a mechanism
to group all menu items that
...
commonly appear in a few different forms:
 ...
 - The [Disclosure Navigation Menu Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/disclosure/examples/disclosure-navigation/)
...
### WHATWG
- HTML spec issue: [Menu elements proposal](https://github.com/whatwg/html/issues/11729)
- HTML spec PR: [Introduce the new menu elements](https://github.com/whatwg/html/pull/12011)
...
### ARIA
- [Draft menu-element accessibility proposal](https://github.com/w3c/aria/issues/2782)
