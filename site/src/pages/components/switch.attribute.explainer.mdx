...
This explainer documents the rationale for the attribute approach: it reuses existing checkbox infrastructure (form participation, label association, keyboard handling, ARIA mapping), stays backwards compatible with older browsers, and covers the vast majority of real-world switch use cases with no JavaScript required.
...
- The use cases requiring child content are rare (see data below)
...
- Design system maintainers (Material UI, Radix, Ant Design, Element Plus) whose APIs informed the user research
