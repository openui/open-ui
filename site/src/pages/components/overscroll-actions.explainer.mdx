...
`overscrollstart` event should be canceled for the open-to-closed transition.
...
and `oldState` fields, similar to `ToggleEvent`, so that developers can determine
the direction of the overscroll before cancelling.
...
the sequential focus navigation order (the tab order). You can refer to
[WCAG 3.2.1 Understanding "On Focus"](https://www.w3.org/WAI/WCAG20/Understanding/on-focus)
which says that focus shouldn't "change the context". If there are interactive
...
and that would activate the area. Typically, overscroll areas such as a drawer
menu represent a context change.
