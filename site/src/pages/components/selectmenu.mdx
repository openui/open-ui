...
In order to resolve these two issues, we could have an attribute opt-in on the select element, like `<select custom>`. However, this has its own issues as well: we would have to make a major parsing change based on the presence of an attribute, which doesn't happen for other attributes/elements. What if a `<select custom>` is parsed and an inline script child of it removes its `custom` attribute before the `<select>` has been closed?

For these reasons, reusing the `<select>` tag for this feature would not be backwards compatible. Creating a new tag name is the best option.
