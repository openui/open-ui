...
- `<breadcrumb>` - The root element that contains the list of items and dividers, it's composed by `nav` and `ol` elements  **[required]**
- `<breadcrumb-item>` - The element representing a place in the path to the current position on the application, it's a focusable `li` that can contain text or anchors **[required]**
- `<breadcrumb-divider>` - The divider between the `<breadcrumb-item />`, it should be a non-focusable li
