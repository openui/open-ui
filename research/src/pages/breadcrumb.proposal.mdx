...
- `<breadcrumb-item>` - The element representing a place in the path to the current position in the application **[required]**
...
The list of Breadcrumb Items should be wrapped by a navigation landmark `nav` containing `aria-label` or `aria-labelledby`, the `<breadcrumb-item />` representing the current page should contain `aria-current="page"` ( if the current page isn't a link the attribute is optional )
