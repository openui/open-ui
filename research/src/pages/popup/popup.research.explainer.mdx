...
Note also that more than one `async` pop-up can use `defaultopen` and all such pop-ups will be shown on load, not just the first one:
...
Pop-up nesting is not posslbe/applicable to the other pop-up types, such as `popup=hint` and `popup=async`.
...
<table>
  ...
  <tr><td></td><td></td><td>Fullscreen</td><td>Modal Dialog</td><td>Pop-up</td><td>Hint</td><td>Async</td></tr>
