...
<p class="question">
  When following these run steps, this effectively means the last popup in the DOM order with
  initiallyopen specified will be the popup that "wins". Is that clear enough to implementers? Is
  that a reasonable behavior?
</p>

### The `show()` method

The `show()` method displays the `popup` element, such that the user can interact with it.
