...
First off, here are the different ways folks contribute to Open UI, and the different skills/pieces of work.
...
A champion has slightly more responsbility than a contributor. A chanmpion is responsible for moving a proposal from one stage to the next, attending meetings, gathering input, synthesizing it, and advancing it through the [stages](/working-mode). In order to become and editor you need to:

- Be a member of the community group
- Be able to alot time to attend weekly meetings while youre feature is being worked on
- Be available to review and advance tasks outside of the weekly meeting

### Editor

An editor owns a specification, and is responsible for ensuring it develops and remains stable. This means reviewing PRs to the specifications you are an editor of, considering input and updating prose in a consistent and accessible voice. In order to become and editor you need to:
...
The following guides walks you through the process of bringing a componant from idea to either a standardized Open UI Web Component and/or an implemented feature in a browser.

### 0. Research: Collect Research and share your Idea

#### Why research

Research is a cornerstone of Open UI. Research enables us to not only ensure all use-cases are covered but the best and most inclusive approaches are taken. Research should not only be limited to component libraries or current web technology; although those are very important. Leverage any and all content and perspectives that you may uncover in order to inform the proposal. Some common resources that people utilize are:
...
1. Look at all the design systems you can get find and see how they treat your componant.
   Create a research page using this context and following the workflow documented in the [Desiogn Systems Analysis page](/design-system-analysis)
...
9. Finally, you should take the output of your research, synthesize it, and make a porposal using the [Proposal Guide](/proposal-guide)

10. Once you have your research and proposal together, it needs approval from two Open UI editors or chairs in order to advance to the next stage. To get this, you should open a pull request. The goal of this process is to allow for asynchronous changes, transparency, and opportunities to ensure consesus from the community. The process is:
...
We write specifications in order to describe how something is going to work, so we can talk about it, build consensus and harmony, drive additional peer review, and support multi implementations agreeing and interoperating.
...
- Create Component Spec: component-spec-template.mdx which will involves specifying the anatomy, behavior and states of a component. Things you should consider in your componant spec:
  ...
  - Are you adding anew element to html? If so it should
    - be described in this spec with the same scope as the HTML spec, though feel free to make it more accessible to understand as a use or a web developer.
    - You’ll also need to add default rendering rules. You can use CSS for this.
    - Make sure to include a consideration for both the browser rendering, and the AT rendering. If you can’t represent your component in ARIA you should also add new ARIA prose to your spec. New elements should also have HTML Accessibility API Mappings (AAM) described, and document conformance requirements (for validators/conformance checkers) specified.
    - New events you envision should be added as well as any additional DOM API you envision.
    - States should be specified as well.
    - Make sure to descrive various interfaces, like keyboard interaction, hit testing rules (where clicks go when you click on it), and otherwise.
    - Make sure to descrive various interfaces, like keyboard interaction, hit testing rules (where clicks go when you click on it), and otherwise.
...
Note that at this stage, we don't necessarily know if the componante will be graduated into the web platform or not. However, if you think it might, or if you have ambitions to do so, it's a good idea to skip ahead to the guidance for Stage #3 "recommendation" to see what your spec will need to look like, and incorporate that guidance into this stage. Doing so will make it easier to advance your componant to stage #4 when that time comes.
...
- ARIA: Add an item to the ARIA WG weekly meeting agenda. Attend the ARIA weekly meeting and share your component accessible interaction model. Get input. [TODO: Add some guidance on how to develop the accessibility model of a new control based on prior art in ARIA.][todo add something about where to raise issies, e.g: authoring practices task force - raise an issue - tagged in x way]

- WHATWG/HTML: TODO
- CSSWG: TODO

- Privacy, and security: Whenever we design a componant it’s a good idea to get input from the privacy community. We strive to reach out to the Privacy Interest Group (PING), as well as the Design Justice Community for a consentful tech analysis. [TODO: add discussion of consent]

- Internationalization: https://github.com/w3c/i18n-activity/issues and https://lists.w3.org/Archives/Public/www-international.

- Architectural coherence: After incorporating feedback from other standards groups, we seek high-level input from TAG about how the componant fits into the technical architecture of the web platform. You can open an issue here: https://github.com/w3ctag/design-reviews/issues.

- Browser Implementers: It's a good idea at this point to start getting feedback from folks who might be implementing support for this (either directly or via the runtime), and get their reactions. You can find some guidance on who to talk to at https://wpc.guide/who-to-talk-to/#browser-engines.
...
The first step of Stage 3 is to determine if this feature is going to be added to the platform directly by browser engine implementers, or implemented in JavaScript, HTML and CSS as as a web componant. For direct implementation, it's often the case that you might first implement a web component. So often the answer may be both.

For the purposesof Open UI, we consider a Web Componant to be an implementation. We still distinguish between "web coponent" and "graduated" components, because of the different work that goes into adding a component to a browser engine.

### 3.2a Implement Web Component

If going this route, it's now time to implement the design you've built consensus on.
...
If you have buy in from browser engines and HTML/CSS/ARIA editors to add the component to he web platform, you'll take these steps:
...
For another example of an Open UI CSS change, see the spectrum of stylability here https://github.com/WICG/open-ui/issues/203.
...
External:
Maybe only \<details open\>. TODO: Ask WHATWG if \<details open\> was a successful experiment that new features (e.g. \<popup\>) should copy?
