---
name: selectmenu explainer
path: /components/selectmenu
showInMenu: true
---
...
The `<select>` element does not provide enough customization for web developers, which leads them to implement their own and likely miss out on the performance, reliability, and accessibility of built-in form control elements. More on that is in the [select proposal](../components/select) and [Custom Control UI](https://github.com/MicrosoftEdge/MSEdgeExplainers/blob/main/ControlUICustomization/explainer.md#introduction).
