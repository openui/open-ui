...
This section relates to a single `<oui-radio>`. Because it cannot stand on its own, without being part of and either explicit or implicit `<oui-radio-group>`, it does not have attributes such `required` or `readonly`, those being attributes of the group. A single `required` option is a group makes the other choices pointless, while a `readonly` option makes itself pointles.

The radio has, however a `disabled` attribute, meaning it's a possible choice, but that is currently unavailable.
