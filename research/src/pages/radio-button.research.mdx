...
_(For a lenghtier discussion, on which this page was based, please refer to the [original radio button issue](https://github.com/WICG/open-ui/issues/251))_

The research showed that different DS’s treat radio buttons in one of two ways, i.e.:

- as the main component, hosted on a “**radio button group**” component;
- as a part of a “**radio button group**” component, which, in fact, is the actual component.

The two approaches seem very similar at first, but the former seems to indicate that single “radio buttons” could exist. The latter, however, shifts the scope up to the “radio button group”, which, although nonexistent as an HTML element, is an important concept for the W3 specification:
...
- Radio Button Group (the actual component) - Group Label (functions as a label for the whole group) - Required Indicator - Radio Button (at least two required) - Checked Indicator (input) - Label (optional) - Radio Button - ...
- ...
...
- In the first case: - whether the first, more rigid proposal should be developed; - or maybe even create an attribute to the `<select>` tag that would make it behave visually like radio buttons
- Or, in the second case, strive for flexibility and: - propose modifications to the labeling of the current radio buttons; - or change the tags used to create them, like in the last example.
